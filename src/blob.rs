...
impl Blob {
    ...
    pub fn digest(&self) -> &str {
        unsafe { CBLBlob_Digest(self._ref).as_str().unwrap() }
    }

    /** The MIME type assigned to the blob, if any. */
    pub fn content_type(&self) -> Option<&str> {
        unsafe { CBLBlob_ContentType(self._ref).as_str() }
    }
    ...
    pub fn properties(&self) -> Dict {
        unsafe {
            Dict {
                _ref: CBLBlob_Properties(self._ref),
                _owner: PhantomData,
            }
        }
    }
    ...
    pub fn open_content(&self) -> Result<BlobReader> {
        check_ptr(
            |err| unsafe { CBLBlob_OpenContentStream(self._ref, err) },
            |stream| BlobReader {
                blob: self,
                _stream_ref: stream,
            },
...
impl Drop for Blob {
    fn drop(&mut self) {
        unsafe {
            release(self._ref as *mut CBLBlob);
        }
    }
}
...
impl Clone for Blob {
    fn clone(&self) -> Self {
        unsafe {
            Blob {
                _ref: retain(self._ref as *mut CBLBlob),
            }
        }
    }
}
...
impl<'r> Drop for BlobReader<'r> {
    fn drop(&mut self) {
        unsafe {
            CBLBlobReader_Close(self._stream_ref);
        }
    }
}
